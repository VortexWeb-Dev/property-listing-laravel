  ...
  return (
    <Card className="overflow-hidden group relative">
      ...
      <CardContent className="p-4">
        <h3 className="font-semibold text-lg mb-1">{property?.title_en || ''}</h3>
        <p className="text-sm text-muted-foreground mb-2">{property?.pf_location?.location || ''}</p>
        <div className="flex justify-between items-center mb-2">
          <span className="font-bold text-lg">{property?.price || '0'} AED</span>
          <Badge variant={property?.status === 'draft' ? 'default' : 'secondary'}>
            {property?.status || ''}
          </Badge>
        </div>
        <div className="flex justify-between text-sm text-muted-foreground">
          <span className="flex items-center"><Bed className="h-4 w-4 mr-1" /> {property?.bedrooms || 'null'}</span>
          <span className="flex items-center"><Bath className="h-4 w-4 mr-1" /> {property?.bathrooms || 'null'}</span>
          <span className="flex items-center"><Square className="h-4 w-4 mr-1" /> {property?.plot_size || 'null'} sqft</span>
        </div>
      </CardContent>
      <CardFooter className="p-4 pt-0 flex justify-between items-center">
        ...
        <span className="text-sm text-muted-foreground">Ref: {property?.reference_number || ''}</span>
      </CardFooter>
      <div className="absolute inset-0 bg-black/70 text-white p-4 flex flex-col justify-between rounded-t-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out" style={{ top: '6rem'}}>
        <div>
